...
//Eventos do Teclado
HANDLE hEventA = CreateEvent(NULL, FALSE, FALSE, "CapturaAlarmes");  //Reset autom�tico e inicializa n�o-sinalizado
...
int main()
{
	...
	hThreadAlarme = (HANDLE)_beginthreadex(
		NULL,
		0,
		(CAST_FUNCTION)FuncAlarme,	// casting necess�rio
		(LPVOID)Alarme,
		0,
	...
	hThreadDados = (HANDLE)_beginthreadex(
		NULL,
		0,
		(CAST_FUNCTION)FuncDados,	// casting necess�rio
		(LPVOID)Dados,
		0,
		(CAST_LPDWORD)&dwThreadId	// cating necess�rio
	);
	...
	hThreadAUX = (HANDLE)_beginthreadex(
		NULL,
		0,
		(CAST_FUNCTION)ConsomeStackPrincipal,	// casting necess�rio
		(LPVOID)iAUX,
		0,
		(CAST_LPDWORD)&dwThreadId	// cating necess�rio
	);
	...
	for (int t = 0; t < 7; ++t) {
		...
		CloseHandle(hThread[t]);	// apaga refer�ncia ao objeto
	}
...
DWORD WINAPI FuncPesagem(LPVOID id)
{
	...
	do {
		for (int p = 1; p <= 999999 && !Interruptores[4]; p++) {
			...
			WaitForSingleObject(hInts[1], INFINITE); //Bloqueia se Sinalizador n�o-sinalizado	
...
DWORD WINAPI FuncCLPalarme(LPVOID id) //Alarme proveniente do clp, mesmo formato de mensagem do alarme de peso
{
	...
	do {
		for (int p = 1; p <= 999999 && !Interruptores[4]; p++) {
			...
			WaitForSingleObject(hInts[2], INFINITE); //Bloqueia se interruptor n�o-sinalizado
			WaitForMultipleObjects(2, Events, TRUE, INFINITE);
...
DWORD WINAPI FuncAlarme(LPVOID id)
{
	while (1) // Espera conex�o
	{
		...
		// Se o handle � v�lido pode usar o pipe
			if (pipe != INVALID_HANDLE_VALUE)
				break;
	}
	
	do {
		WaitForSingleObject(hInts[0], INFINITE); //Bloqueia se interruptor n�o sinalizado
...
DWORD WINAPI FuncDados(LPVOID id) //Captura os dados do processo da lista circular e envia para a tarefa "ExibeDados"
{
	hFile = CreateFile("processo.txt",
		...
		OPEN_ALWAYS,// cria novo arquivo caso ele n�o exista, abre se j� existe
		FILE_ATTRIBUTE_NORMAL,
		NULL);		// Template para atributos e flags
	if (hFile == INVALID_HANDLE_VALUE)  std::cerr << "\nErro na cria��o do arquivo = " << GetLastError() << "\n";
	
	int indice = 0;
	WaitForSingleObject(hTimeOut, 20);
	do {
		WaitForSingleObject(hInts[3], INFINITE); //Bloqueia se interruptor n�o sinalizado
		...
		if (!isemptyCLP() && !Interruptores[4]) {
			...
			// Atualiza a posi��o para leitura
